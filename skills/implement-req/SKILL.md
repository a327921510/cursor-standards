...
4. 实现保持最小改动；遵守各仓已有 coding rules（含可读性：单一编排入口、调用深度 ≤3、未复用不抽离）。
5. **用户可见主路径**（新能力或改了主流程）：在编排入口补 **流程地图**（stage 1→N）与 `// --- stageName ---` 分段；stage 名短英文 camelCase，与日志 `stage` 字段一致。旁路不上主地图。不要为对齐测试去拆一堆只调用一次的函数。主路径未定时不要顺手铺 E2E / 全量 `data-testid`。
6. Commit（仅当用户明确要求提交时）：Conventional Commits，footer 写同一 `REQ-ID`；多仓各自 commit 都带该 ID。
