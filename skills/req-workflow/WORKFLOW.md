...
> 细则 skill：`req-register`（登记）/ `implement-req`（实现）/ `sync-req-docs`（归档）
...
| 详细测试用例（可选） | 你 / Agent | **代码确认并归档（阶段 B）之后**，若要认真详测或给他人测，再生成步骤级用例（见 §7 步骤 ⑤）；不等于单元测试 |

**验收标准 vs 详细测试用例**：前者是 REQ 里「做成什么样算完」的短清单；后者是可选的步骤 + 预期结果清单，覆盖边界。小需求默认只靠验收标准即可。
...
日常和 Cursor 对话时，可按下面推进——与上文 Step 1～5 **同一条主流程**，只是把「写台账 / 写码 / 归档 /（可选）测试用例」交给 Agent。
...
- ①→②→④ 与文档一致：**先留痕再写码，确认后再归档**；⑤ 在 ④ 之后、正式「已上线」之前按需做。
...
- 改完用中文简报：端、文件、入口路径、stage 表、调用深度、如何自测验收句；未确认前不要 git commit
```
